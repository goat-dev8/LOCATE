...
export async function verifyReceiptInBrowser(signature, rpc, programId) {
    ...
    const confirmed = finalized ??
        (await connection.getTransaction(signature, {
            commitment: "confirmed",
            maxSupportedTransactionVersion: 0,
        }));
    if (!confirmed)
        return { ok: false, status: "rejected", reason: "NOT_FOUND" };
    if (confirmed.meta?.err)
        return { ok: false, status: "rejected", reason: "TRANSACTION_FAILED" };
